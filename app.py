...
# --- SECRETS & FILES ---
# Zieht die Discord-URL jetzt sicher aus den Secrets!
...
# Vorgaben aus dem Profil angewendet
...
# --- ZAHLENFORMATIERUNG ---
# Angewandte Regel: Nur noch Punkt, kein Komma für Euro-Beträge
def fmt(val, dec=0):
    if dec > 0:
        return f"{val:.{dec}f}"
    else:
...
# --- AUTO REFRESH ---
# Aktualisiert die App alle 60 Sekunden nahtlos
st_autorefresh(interval=60000, key="data_refresh")

# --- ROBUSTE API ABFRAGE (KURS & VORTAG) ---
def fetch_market_data(ticker_symbol):
    ...
    try:
        ...
        if not df_hist.empty and "Close" in df_hist.columns:
            ...
            if len(df_clean) >= 2:
                ...
                # Erfolgreichen Abruf lokal cachen
    ...
    # Fallback auf den lokalen Cache
    ...
    # Absoluter Notfall: Nutze Anfangskurs
    if live_kurs is None:
        return ANFANGSKURS, ANFANGSKURS, "N/A", "Keine Daten"
...
aktueller_kurs, vortag_kurs, vortag_datum_str, api_status = fetch_market_data(api_symbol)

# --- SIDEBAR & PROGNOSE EINSTELLUNGEN ---
st.sidebar.markdown("### ⚡ System Status")
if "Cached" in api_status or "Fehler" in api_status or "Offline" in api_status:
...
# --- CHART DATEN (ECHTE HISTORIE STATT RANDOM) ---
...
def get_real_chart_data(ticker_symbol):
    ...
    # Notfall-DataFrame falls Yahoo offline ist, damit die App nicht crasht
...
# Meilenstein Berechnung auf Basis von echter erwarteter Rendite
sim_b = brutto_ist
monate_bis_ziel = 0
...
with tab_forecast:
    ...
    forecast_data.append({
        ...
        "Brutto Depotwert": f"{fmt(STARTKAPITAL, 2)} €", "Gesamter Gewinn": "+0.00 €",
        "Netto Depotwert": f"{fmt(STARTKAPITAL, 2)} €", "Kumulierte Entnahme": "0.00 €",
    })
    forecast_data.append({
        "Index": 1, "Jahr": "Heute", "Datum": heute_date.strftime("%d.%m.%Y"),
        "Brutto Depotwert": f"{fmt(brutto_ist, 2)} €", "Gesamter Gewinn": f"+{fmt(gewinn_brutto, 2)} €",
        "Netto Depotwert": f"{fmt(netto_ist, 2)} €", "Kumulierte Entnahme": f"{fmt(gesamt_entnommen, 2)} €",
    })

    sim_b_prog, sim_n_prog, sim_e_prog = brutto_ist, netto_ist, gesamt_entnommen
    for m_idx in range(1, 121): # Simulation für 10 Jahre
        ...
        if m_idx % 12 == 0:
            forecast_data.append({
                ...
                "Brutto Depotwert": f"{fmt(sim_b_prog, 2)} €",
                "Gesamter Gewinn": f"+{fmt(sim_b_prog - STARTKAPITAL, 2)} €",
                "Netto Depotwert": f"{fmt(sim_n_prog, 2)} €",
                "Kumulierte Entnahme": f"{fmt(sim_e_prog, 2)} €",
            })
