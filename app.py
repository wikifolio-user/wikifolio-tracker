...
def send_discord_alert(pct_change, current_price):
    ...
    if last_alert_time and (now - last_alert_time).total_seconds() < 3600: return False
    msg = f"🚨 **QUANT TERMINAL ALARM** 🚨\nDas Wikifolio **{WKN}** ({ISIN}) ist stark gefallen!\nTagesveränderung: **{pct_change:+.2f}%**\nAktueller Kurs: **{current_price:.3f} €**"
    payload = {"content": msg}
    try:
...
aktueller_kurs_input = 302.100 if not api_kurs else api_kurs
vortag_kurs = 301.250 if not api_vortag else api_vortag
vortag_datum_str = api_vortag_datum if api_vortag_datum != "N/A" else (datetime.datetime.now(BERLIN_TZ) - datetime.timedelta(days=1)).strftime("%d.%m.%Y")

st.sidebar.markdown("### ⚡ Vollautomatischer Live-Sync")
if api_kurs and api_vortag:
    st.sidebar.success(f"🟢 Verbunden ({api_status})\n\nKurs: {api_kurs:.3f} €\nVortag: {api_vortag:.3f} €")
else: st.sidebar.warning("⚠️ API eingeschränkt. Nutze intelligente Fallbacks.")
...
<style>
    ...
    .header-bar { display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px; background: #09090B; border: 1px solid #27272A; border-left: 3px solid #00C853; border-radius: 6px; padding: 12px 16px; margin-bottom: 16px; }
    ...
    #MainMenu, footer, header { visibility: hidden; } .block-container { padding-top: 0.8rem; padding-bottom: 4rem; }
</style>
...
if tages_verenderung_pct <= -1.0: send_discord_alert(tages_verenderung_pct, aktueller_kurs)

start_dt = pd.to_datetime("2025-07-09")
def berechne_monatliche_entnahme(dt):
    monate = (dt.year - start_dt.year) * 12 + (dt.month - start_dt.month)
    if dt.day < start_dt.day: monate -= 1
...
if now_berlin.day < start_dt.day: heutige_monate_anzahl -= 1
...
rendite_mo = (((aktueller_kurs / ANFANGSKURS) ** (1 / max(1, monate_gehalten))) - 1) * 100
...
if not milestone_reached:
    for m_i in range(1, 120):
        ...
        if sim_b >= 100000.0:
            meilenstein_datum_str = (now_berlin + pd.DateOffset(months=m_i)).strftime("%m.%Y")
            break
    if not milestone_reached and sim_b < 100000.0: meilenstein_datum_str = "> 10 Jahre"
...
<div class="grid-container">
    ...
    <div class="m-card">
        ...
        <div class="m-sub pos">+{fmt(gewinn_brutto)} € ({rendite_ist_pct:.2f}%)</div>
    </div>
...
with tab_future:
    ...
    st.info(f"Basis: {fmt(STARTKAPITAL, 2)} € Startkapital | {fmt(ENTNAHME_PM, 2)} € Entnahme/Monat")
    ...
    for zins in zinssaetze:
        ...
        jahreswerte = {}
        ziel_monat = "n.e."
        jahreszins_eff = ((1 + zins) ** 12 - 1) * 100
        for m in range(1, 61):
            kapital = kapital * (1 + zins) - ENTNAHME_PM
            if kapital >= 100000.0 and ziel_monat == "n.e.": ziel_monat = f"Monat {m}"
            if m % 12 == 0: jahreswerte[f"Jahr {m//12}"] = kapital
        row = {"Zins p.M.": f"{zins*100:.1f}%".replace(".", ","), "Zins p.a.": f"{jahreszins_eff:.2f}%".replace(".", ","), **jahreswerte, "Ziel 100k": ziel_monat}
        data.append(row)

    df_zukunft = pd.DataFrame(data)
    cols_to_format = ["Jahr 1", "Jahr 2", "Jahr 3", "Jahr 4", "Jahr 5"]
    styled_df = df_zukunft.style.format({col: lambda x: f"{fmt(x, 2)} €" for col in cols_to_format}).map(
        lambda x: "background-color: #0c1410; color: #00C853; font-weight: bold;" if isinstance(x, (int, float)) and x >= 100000.0 else "",
